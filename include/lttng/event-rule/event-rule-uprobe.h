...
/*
 * Set the filter expression of a uprobe event rule.
 *
 ...
 * if invalid parameters are passed.
 */
extern enum lttng_event_rule_status lttng_event_rule_uprobe_set_filter(
		struct lttng_event_rule *rule, const char *expression);

/*
 * Get the filter expression of a uprobe event rule.
 *
